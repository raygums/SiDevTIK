...
<div class="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            ...
            <p class="mt-2 text-gray-600">Kelola notifikasi aktivitas sistem</p>
        </div>
        @if($unreadCount > 0)
    ...
    <div class="mb-6 grid gap-4 sm:grid-cols-3">
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-4">
            ...
            <p class="mt-2 text-2xl font-bold text-success">{{ $notifications->total() - $unreadCount }}</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        <div class="p-12 text-center">
            ...
            <p class="mt-2 text-sm text-gray-500">Semua notifikasi sudah dilihat atau belum ada aktivitas</p>
        </div>
        @else
        <ul class="divide-y divide-gray-200">
            @forelse($notifications as $notification)
            <li class="hover:bg-gray-50 transition">
                <a href="{{ route('admin.notifications.show', $notification) }}" class="block p-4 sm:p-6">
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    ...
                                    <div class="mt-2 text-xs text-gray-500">
                                        👤 {{ $notification->relatedUser->nm }} ({{ $notification->relatedUser->email }})
                                    </div>
                                    @endif

                                    @if($notification->relatedSubmission)
                                    <div class="mt-1 text-xs text-gray-500">
                                        📋 Tiket: <span class="font-mono">{{ $notification->relatedSubmission->no_tiket }}</span>
                                    </div>
                                    @endif
                                </div>
